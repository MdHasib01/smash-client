...
type NavEntry = { name: string; to: string; icon: React.ComponentType<{ size?: number; className?: string }>; action?: string };
...
export const Sidebar: React.FC = () => {
  ...
  const NavItem: React.FC<{ item: NavEntry }> = ({ item }) => {
    ...
    const className = cn(
      "relative flex items-center gap-3 px-3 py-2 rounded-xl transition-all duration-200 group text-left w-full outline-none focus-visible:ring-[3px] focus-visible:ring-ring/40",
      isActive
        ? "bg-white/10 text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.1)]"
        : "text-smash-text-secondary hover:text-white hover:bg-white/5"
    );
    ...
    const content = (
      <>
        ...
        {isNotifications && (
          <div className="absolute top-2 left-[26px] w-2 h-2 rounded-full bg-rose-500 ring-2 ring-smash-base" />
        )}
    ...
    if (!isSidebarCollapsed) return link;
  ...
  return (
    <motion.aside
      ...
      className="hidden md:flex h-full shrink-0 flex-col gap-6 glass-1 border-y-0 border-l-0 border-r border-white/5 py-6 px-4 relative z-40 transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)]"
    >
      ...
      <nav className="flex flex-col gap-1 flex-1 overflow-y-auto overflow-x-hidden no-scrollbar px-2">
        {!isSidebarCollapsed && <div className="text-[10px] uppercase tracking-widest text-smash-text-tertiary mb-2 font-bold mt-2">Main Menu</div>}
      ...
      <button
        ...
        className="absolute -right-3 top-8 w-6 h-6 rounded-full bg-smash-surface border border-white/10 flex items-center justify-center text-smash-text-secondary hover:text-white hover:border-[#D946EF]/50 hover:scale-110 transition-all z-50 shadow-lg"
      >
