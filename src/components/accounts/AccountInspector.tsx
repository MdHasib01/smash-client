    ...
    <div className="flex flex-col gap-6 w-full text-white pb-10">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl glass-3 flex items-center justify-center font-bold text-lg text-[#D946EF] border border-[#D946EF]/30">
            {connection.provider.slice(0, 2)}
          </div>
          <div className="flex flex-col flex-1">
            <h3 className="font-bold text-lg tracking-tight leading-none mb-1">{connection.name}</h3>
        ...
        <div className="glass-3 rounded-xl p-3 flex justify-between items-center border border-white/5 mt-2">
          <span className="text-[10px] font-bold tracking-widest uppercase text-smash-text-tertiary">Current Status</span>
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <h4 className="text-[10px] font-bold tracking-widest uppercase text-smash-text-secondary">Capabilities</h4>
          ...
          {['IMAGE', 'VIDEO', 'TEXT', 'AUDIO'].map(cap => {
            ...
            return (
              <div key={cap} className="flex items-center justify-between p-2 rounded-lg hover:bg-white/5 transition-colors">
                ...
                {/* Display-only: capabilities are auto-detected. */}
                <Switch checked={hasCap} tabIndex={-1} aria-readonly className="pointer-events-none" aria-label={`${cap} capability`} />
              </div>
            );
          })}
        </div>
      ...
      {connection.type === 'BROWSER' && (
        <div className="flex flex-col gap-3">
          <h4 className="text-[10px] font-bold tracking-widest uppercase text-smash-text-secondary">Session Isolation</h4>
          <div className="glass-2 rounded-xl border border-white/5 p-4 flex flex-col gap-4">
            ...
            <Button variant="primary" className="w-full mt-2"><Globe size={14} /> Open Secure Session</Button>
          </div>
        </div>
      )}
      ...
      {connection.type === 'API' && (
        <div className="flex flex-col gap-3">
          <h4 className="text-[10px] font-bold tracking-widest uppercase text-smash-text-secondary">Authentication</h4>
          <div className="glass-2 rounded-xl border border-white/5 p-4 flex flex-col gap-4">
            <div className="flex flex-col gap-1">
              ...
              <div className="flex items-center justify-between bg-white/5 px-3 py-2 rounded-lg border border-white/10">
                ...
                <Button variant="ghost" size="xs" className="text-[9px] tracking-wider">REPLACE</Button>
              </div>
            </div>
      ...
      {/* Health & Routing */}
      <div className="flex flex-col gap-3">
        <h4 className="text-[10px] font-bold tracking-widest uppercase text-smash-text-secondary">Health & Routing</h4>
        <div className="glass-2 rounded-xl border border-white/5 p-4 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
          ...
          <div className="flex items-center justify-between">
            ...
            <Switch checked tabIndex={-1} aria-readonly className="pointer-events-none" aria-label="Auto-Reconnect" />
          </div>
